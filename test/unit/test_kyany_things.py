...
def test_valid_kyan_address():
    from kyanlib import is_valid_kyan_address

    main = valid_kyan_address()
    test = valid_kyan_address('testnet')

    assert is_valid_kyan_address(main) is True
    assert is_valid_kyan_address(main, 'mainnet') is True
    assert is_valid_kyan_address(main, 'testnet') is False

    assert is_valid_kyan_address(test) is False
    assert is_valid_kyan_address(test, 'mainnet') is False
    assert is_valid_kyan_address(test, 'testnet') is True


def test_invalid_kyan_address():
    from kyanlib import is_valid_kyan_address

    main = invalid_kyan_address()
    test = invalid_kyan_address('testnet')

    assert is_valid_kyan_address(main) is False
    assert is_valid_kyan_address(main, 'mainnet') is False
    assert is_valid_kyan_address(main, 'testnet') is False

    assert is_valid_kyan_address(test) is False
    assert is_valid_kyan_address(test, 'mainnet') is False
    assert is_valid_kyan_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from kyanlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % kyanlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(kyanlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(kyanlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(kyanlib.blocks_to_seconds(16616)) == 2612035
