...
def is_valid_kyan_address(address, network='mainnet'):
    ...
    kyan_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Kyan address
        return False

    if (address_version != kyan_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('kyan.conf')
