...
class KyanDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some kyand methods
    ...
    @classmethod
    def from_kyan_conf(self, kyan_dot_conf):
        from kyan_config import KyanConfig
        config_text = KyanConfig.slurp_config_file(kyan_dot_conf)
        creds = KyanConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = kyanlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = kyanlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = kyanlib.blocks_to_seconds(diff)
