...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, kyand):
        golist = kyand.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_kyand(kyand, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_kyand(self, kyand, rec):
        import decimal
        import kyanlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from kyand...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from kyand, with every run
        ...
        # get/create, then sync payment amounts, etc. from kyand - Kyand is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(kyand)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from kyand! %s" % e)
            govobj.vote_delete(kyand)
            return (govobj, None)
    ...
    def vote_delete(self, kyand):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(kyand, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, kyand, signal, outcome):
        import kyanlib
        ...
        output = kyand.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = kyanlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(kyand, signal)

    def sync_network_vote(self, kyand, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = kyand.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = KYAND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 kyan addr, non-multisig
            if not kyanlib.is_valid_kyan_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Kyan address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Kyan Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (kyanlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = KYAND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not kyanlib.is_valid_kyan_address(addr, config.network):
    ...
    def hash(self):
        import kyanlib
        return kyanlib.hashit(self.serialise())
